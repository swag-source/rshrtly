...
function isValidCustomUrl(custom_url: string | undefined): boolean {
    ...
    const reservedWords = ['api', 'admin', 'www', 'app', 'dashboard', 'analytics'];
...
export async function hashURL(req: any, res: any) {
    try {
        ...
        if (!isValidCustomUrl(customUrl)) {
            return res.status(400).json({
                "Error": 'Custom URL must be 3-50 characters, alphanumeric with hyphens/underscores only'
            });
        }
        ...
        if (customUrl) {
            const customCheck = await pool.query(
                'SELECT custom_url FROM urls WHERE custom_url = ?', [customUrl]
            ) as any;
